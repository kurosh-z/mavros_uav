...
from mavros_msgs.srv import CommandBool, ParamGet, ParamSet, SetMode, WaypointClear, WaypointPush, SetModeRequest
...
class MavrosTestCommon(object):
    ...
    def setUp(self):
        ...
        self.sub_topics_ready = {
            key: False
            for key in ["alt", "ext_state", "global_pos", "home_pos", "local_pos", "mission_wp", "state", "imu"]
        }
        ...
        try:
            ...
            rospy.logfatal("failed to connect to services")
    ...
    def set_arm(self, arm, timeout):
        ...
        if arm_set:
            rospy.loginfo("arm_se sucesseded ")
        else:
            rospy.logerr("arm_set FAILED!")
        # self.assertTrue(
        #     arm_set,
    ...
    def set_mode(self, mode, timeout):
        ...
        for i in xrange(timeout * loop_freq):
            ...
            try:
                ...
                rospy.logerr("{}".format(e))
                # self.fail(e)
        rospy.loginfo("mode_set: {}".format(mode_set))
        # self.assertTrue(
        #     mode_set,
        #     (
    ...
    def set_param(self, param_id, param_value, timeout):
        ...
        for i in xrange(timeout * loop_freq):
            try:
                ...
                if res.success:
                    rospy.loginfo(
                        "param {0} set to {1} | seconds: {2} of {3}".format(param_id, value, i / loop_freq, timeout)
                    )
                break
            except rospy.ServiceException as e:
                rospy.logerr("{}".format(e))
            ...
            try:
                ...
                rospy.logerr("{}".format(e))
        rospy.loginfo(f"set_param_srv -> {res.success}")
        # self.assertTrue(
        #     res.success,
        #     (
    ...
    def wait_for_landed_state(self, desired_landed_state, timeout, index):
        ...
        for i in xrange(timeout * loop_freq):
            ...
            try:
                ...
                rospy.logerr("{}".format(e))
        rospy.loginfo("landed_state_confirmed: {}".format(landed_state_confirmed))
        # self.assertTrue(
    ...
    def wait_for_vtol_state(self, transition, timeout, index):
        ...
        for i in xrange(timeout * loop_freq):
            ...
            try:
                ...
                rospy.logerr("{}".format(e))
                # self.fail(e)
        rospy.loginfo("transitioned: ", transitioned)
        # self.assertTrue(
        #     transitioned,
        #     (
    ...
    def clear_wps(self, timeout):
        ...
        for i in xrange(timeout * loop_freq):
            ...
            try:
                ...
                rospy.logerr("{}".format(e))
                # self.fail(e)
        rospy.loginfo("wps_cleared: ", wps_cleared)
    ...
    def send_wps(self, waypoints, timeout):
        ...
        for i in xrange(timeout * loop_freq):
            ...
            try:
                ...
                rospy.logerr("{}".format(e))  # self.fail(e)

        rospy.loginfo("wps_sent and wps_verified -> ", wps_sent and wps_verified)
        # self.assertTrue(
                ...
                if res.success:
                    ...
                    rospy.loginfo(
                        "MAV_TYPE received | type: {0} | seconds: {1} of {2}".format(
                            mavutil.mavlink.enums["MAV_TYPE"][self.mav_type].name, i / loop_freq, timeout
                        )
                    )
                    break
            except rospy.ServiceException as e:
...
class MavrosOffboardPosctlTest(MavrosTestCommon):
    ...
    def reach_position(self, x, y, z, timeout, idx=None):
        """timeout(int): seconds"""
        ...
        rospy.loginfo(
            "attempting to reach position idx:{0}| x: {1}, y: {2}, z: {3} | current position x: {4:.2f}, y: {5:.2f}, z: {6:.2f}".format(
                idx,
                x,
                y,
                z,
        ...
        for i in xrange(timeout * loop_freq):
            if self.is_at_position(
                self.pos.pose.position.x, self.pos.pose.position.y, self.pos.pose.position.z, self.radius
            ):
            ...
            try:
                ...
                rospy.loginfo("reach error: {}".format(e))
    ...
    def test_posctl(self, positions):
        ...
        # positions = ((0, 0, 0), (50, 50, 20), (50, -50, 20), (-50, -50, 20), (0, 0, 20))

        for i in xrange(len(positions)):
            self.reach_position(positions[i][0], positions[i][1], positions[i][2], 30, i)
            # rospy.sleep(1.5)

        rospy.loginfo("mission finished landing ...")
        self.set_mode("AUTO.LOITER", 3)
        rospy.sleep(5)
...
def generate_circle_traj(radius=0.8, numSteps=100, altitude=2):
    numPoints = 1 * numSteps
    # Create [x,y] points along a circle with specified radius
    PI = np.pi
    xpoints = np.flip(radius * np.sin(np.linspace(-PI, PI, num=numPoints)))
    ypoints = np.flip(radius * np.cos(np.linspace(-PI, PI, num=numPoints)) - radius)
    zpoints = np.flip(altitude * np.ones((numPoints,)))

    posi = list(zip(xpoints.tolist(), ypoints.tolist(), zpoints.tolist()))
    return [(0, 0, 0)] + posi + [(0, 0, 0)]


if __name__ == "__main__":
    import rostest

    rospy.init_node("pctl_node", anonymous=True)

    # rostest.rosrun(PKG, "mavros_offboard_posctl_test", MavrosOffboardPosctlTest)
    px4ctl = MavrosOffboardPosctlTest()
    px4ctl.setUp()
    # positions = [(0, 0, 0), (3, 3, 2), (5, -5, 2), (-5, -5, 2), (0, 0, 2.5)]

    # posi = generate_circle_traj()
    posi = [(0, 0, 1.7), (0, 0, 1)]
    px4ctl.test_posctl(posi)
